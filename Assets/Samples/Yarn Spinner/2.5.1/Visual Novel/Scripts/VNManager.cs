...
/// 让指定的精灵变暗
/// </summary>
/// <param name="spriteName">要操作的精灵名称</param>
/// <param name="darkenFactor">变暗因子，取值范围 0.0 - 1.0，值越小越暗</param>
public void DarkenSprite(string spriteName, float darkenFactor = 0.5f)
{
    Image sprite = FindActorOrSprite(spriteName);
    if (sprite != null)
    {
        ...
        sprite.color = new Color(sprite.color.r * darkenFactor, sprite.color.g * darkenFactor, sprite.color.b * darkenFactor, sprite.color.a);
    }
    else
    {
        Debug.LogErrorFormat(this, "VN Manager couldn't find a sprite with name \"{0}\", maybe it was misspelled or the sprite was hidden / destroyed already", spriteName);
    }
}
	/// <summary>
/// 让指定的精灵变小
/// </summary>
/// <param name="spriteName">要操作的精灵名称</param>
/// <param name="scaleFactor">缩放因子，取值范围 0.0 - 1.0，值越小精灵越小</param>
public void ShrinkSprite(string spriteName, float scaleFactor = 0.5f)
{
    Image sprite = FindActorOrSprite(spriteName);
    if (sprite != null)
    {
        ...
        {
            originalScales[sprite] = sprite.rectTransform.localScale;
        }
        sprite.rectTransform.localScale = new Vector3(sprite.rectTransform.localScale.x * scaleFactor, 
                                                      sprite.rectTransform.localScale.y * scaleFactor, 
                                                      sprite.rectTransform.localScale.z);
    }
    else
    {
        Debug.LogErrorFormat(this, "VN Manager couldn't find a sprite with name \"{0}\", maybe it was misspelled or the sprite was hidden / destroyed already", spriteName);
...
/// 恢复指定精灵的原始颜色和缩放比例
/// </summary>
/// <param name="spriteName">要恢复的精灵名称</param>
public void RestoreSprite(string spriteName)
{
    Image sprite = FindActorOrSprite(spriteName);
    if (sprite != null)
    {
        if (originalColors.ContainsKey(sprite))
        {
            sprite.color = originalColors[sprite];
        }
        if (originalScales.ContainsKey(sprite))
        {
            sprite.rectTransform.localScale = originalScales[sprite];
        }
    }
    else
    {
...
runner.AddCommandHandler<string, float>("DarkenSprite", DarkenSprite);
runner.AddCommandHandler<string, float>("ShrinkSprite", ShrinkSprite);
runner.AddCommandHandler<string>("RestoreSprite", RestoreSprite);
